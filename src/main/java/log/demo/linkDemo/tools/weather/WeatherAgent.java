package log.demo.linkDemo.tools.weather;
...
public class WeatherAgent implements Agent {
    ...
    private String buildNowReport(String userId, String city) {
        ...
        if (now == null) {
            saveQuery(userId, "", city, "now", null, "FAILED", elapsed);
        ...
        saveQuery(userId, "", city, "now", report, "SUCCESS", elapsed);
        return report;
    }
    ...
    private String buildForecastReport(String userId, String city, int dayOffset) {
        ...
        if (forecasts == null || forecasts.days == null || forecasts.days.length <= dayOffset) {
            saveQuery(userId, "", city, "forecast", null, "FAILED", elapsed);
            return "暂无该日天气预报数据";
        }
        ...
        saveQuery(userId, "", city, "forecast", report, "SUCCESS", elapsed);
        return report;
    }
    ...
    private String buildMultiDayReport(String userId, String city) {
        ...
        if (forecasts == null) {
            saveQuery(userId, "", city, "multi-day", null, "FAILED", elapsed);
            return "天气服务暂不可用，请稍后重试";
        }
        ...
        saveQuery(userId, "", city, "multi-day", report, "SUCCESS", elapsed);
        return report;
    }
    ...
    /** 安全获取字符串字段，key 缺失或 value 为 null 时返回默认值 */
    ...
    /** 安全获取数字字段，key 缺失或 value 为 null 时返回默认值 */
